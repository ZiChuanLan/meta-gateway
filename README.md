...
| **上游不是标准 OpenAI 形态** | 渠道级自定义端点与字段映射：任意路径重写 + 请求/响应体字段搬运（同一套 JSON-path 语法） |
...
<table width="100%">
  ...
  <tr>
    ...
    <td width="33.3%" valign="top">
      ...
      <p><b>独立进程沙箱隔离</b> · 一键拉取并安装市场扩展；独立 sidecar 进程托管，环境变量白名单隔离核心凭证。插件可声明拦截钩子，在选路、上游请求与响应三个环节介入。</p>
    </td>
  </tr>
  <tr>
    <td width="33.3%" valign="top">
...
- **端点路径覆盖 / 映射**：`systemone` 这种单段名字进 `/v1` 位（`/v1/systemone`）；带 `/` 或前导 `/`
  ...
  值可用 `{path}`/`{model}`。**映射一旦存在，`<base>/v1/<path>` 的自动补全就不再参与**——这正是根路径
  不是 `/v1` 的供应商需要的。
