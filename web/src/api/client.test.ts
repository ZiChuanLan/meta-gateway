...
describe('ApiClient', () => {
  ...
  it('returns a stable error without retaining response details', async () => {
    ...
    const error = await new ApiClient('do-not-leak').get('/admin/sites').catch((value: unknown) => value)
  ...
  it('normalizes network failures', async () => {
    ...
    await expect(new ApiClient('token').get('/admin/sites')).rejects.toMatchObject({ status: 0, message: 'Unable to reach Meta Gateway' })
  })
  ...
  it('invalidates the session on any unauthorized response', async () => {
    ...
    await expect(new ApiClient('token', onUnauthorized).get('/admin/sites')).rejects.toMatchObject({ status: 401 })
