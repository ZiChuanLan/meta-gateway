...
/** Localized labels for the endpoints a capability can unlock. */
...
/**
 * The model capability registry, as a popup on the Models page. It used to be a
 * tab in the workbench, which put it next to the run-it surface rather than
 * next to the model list it describes; capabilities are catalog data, so they
 * belong with the catalog.
 *
 * Every row here is what routing plans against — a wrong `endpoints` value is
 * silently unplannable, which is why the edit form spells out that the value
 * is not validated on write.
 */
export function CapabilityRegistryDialog({ onClose }: { onClose: () => void }) {
  const { t } = useI18n();
  ...
  return (
    <Dialog title={t("workbench.cap.title")} onClose={onClose} danger={false}>
      <p className="panel-hint">{t("workbench.cap.help")}</p>
      <p className="panel-hint">{t("workbench.cap.desc")}</p>
      <div className="cap-registry-bar">
        <label className="directory-search cap-registry-filter">
          <Search size={14} aria-hidden="true" />
          <input
            value={filter}
            onChange={(event) => setFilter(event.target.value)}
            placeholder={t("workbench.cap.filter")}
            aria-label={t("workbench.cap.filter")}
          />
        </label>
        <input
          value={newModel}
          placeholder={t("workbench.cap.addPlaceholder")}
          aria-label={t("workbench.cap.addPlaceholder")}
      ...
      {items.length > 0 && visible.length === 0 ? (
        <Empty>{t("workbench.cap.noMatch", { term: filter.trim() })}</Empty>
      ) : null}
      {visible.length > 0 ? (
        <div className="cap-registry-list">
          {visible.map((item) => (
            <article className="catalog-plan-row" key={item.model}>
              <header>
              ...
              <p className="cap-registry-meta muted">
                ...
                {item.max_input_images
                  ? ` · ${t("workbench.cap.maxImagesShort", {
                      n: item.max_input_images,
                    })}`
                  : ""}
                {` · ${item.input_modalities.join("/") || "—"} → ${
...
function CapabilityDialog({
  ...
  return (
    <Dialog
      title={t("workbench.cap.editTitle", { name: value.model })}
      onClose={onClose}
      busy={pending}
      actions={
        <>
          ...
          <Button loading={pending} onClick={() => onSave(form)}>
      ...
      <div className="meta-form">
        ...
        <Field label={t("workbench.cap.maxImages")}>
          <input
            ...
            onChange={(e) =>
              patch({
                max_input_images: Math.max(0, Number(e.target.value) || 0),
              })
            }
          />
        </Field>
