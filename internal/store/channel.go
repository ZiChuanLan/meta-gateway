...
func (s *ChannelStore) ListOverviews(now time.Time) ([]domain.ChannelOverview, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&overview.Channel.NonStreamTimeoutSeconds,
			&overview.Channel.StreamPolicy,
...
func (s *ChannelStore) ListEnabled() ([]domain.Channel, error) {
	rows, err := s.db.Query(`SELECT id, site_id, credential_id, name, base_url, models_csv, group_name, priority, weight, status, type_hint, max_reasoning_effort, payload_rules, max_concurrent, non_stream_timeout_seconds, stream_policy, proxy_url, header_override, system_prompt, retry_config, consecutive_failures, stable_first, stable_first_requests, model_sync_mode, created_at, updated_at FROM channels WHERE status = ? ORDER BY priority, id`, domain.StatusEnabled)
	if err != nil {
...
func (s *ChannelStore) ListAutoDisabled() ([]domain.Channel, error) {
	rows, err := s.db.Query(`SELECT id, site_id, credential_id, name, base_url, models_csv, group_name, priority, weight, status, type_hint, max_reasoning_effort, payload_rules, max_concurrent, non_stream_timeout_seconds, stream_policy, proxy_url, header_override, system_prompt, retry_config, consecutive_failures, stable_first, stable_first_requests, model_sync_mode, created_at, updated_at FROM channels WHERE status = ? ORDER BY id`, domain.StatusAutoDisabled)
	if err != nil {
...
func (s *ChannelStore) ListProbeable() ([]domain.Channel, error) {
	rows, err := s.db.Query(`SELECT id, site_id, credential_id, name, base_url, models_csv, group_name, priority, weight, status, type_hint, max_reasoning_effort, payload_rules, max_concurrent, non_stream_timeout_seconds, stream_policy, proxy_url, header_override, system_prompt, retry_config, consecutive_failures, stable_first, stable_first_requests, model_sync_mode, created_at, updated_at FROM channels WHERE status IN (?, ?) ORDER BY priority, id`, domain.StatusEnabled, domain.StatusAutoDisabled)
	if err != nil {
...
func (s *ChannelStore) GetByID(id int64) (*domain.Channel, error) {
	row := s.db.QueryRow(`SELECT id, site_id, credential_id, name, base_url, models_csv, group_name, priority, weight, status, type_hint, max_reasoning_effort, payload_rules, max_concurrent, non_stream_timeout_seconds, stream_policy, proxy_url, header_override, system_prompt, retry_config, consecutive_failures, stable_first, stable_first_requests, model_sync_mode, created_at, updated_at FROM channels WHERE id = ?`, id)
	var r domain.Channel
...
func (s *ChannelStore) Create(c *domain.Channel) (int64, error) {
	...
	res, err := s.db.Exec(`INSERT INTO channels (site_id, credential_id, name, base_url, models_csv, group_name, priority, weight, status, type_hint, max_reasoning_effort, payload_rules, max_concurrent, non_stream_timeout_seconds, stream_policy, proxy_url, header_override, system_prompt, retry_config, stable_first, model_sync_mode) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		c.SiteID, c.CredentialID, c.Name, c.BaseURL, c.ModelsCSV, c.GroupName, c.Priority, c.Weight, c.Status, c.TypeHint, c.MaxReasoningEffort, c.PayloadRules, c.MaxConcurrent, c.NonStreamTimeoutSeconds, c.StreamPolicy, c.ProxyURL, c.HeaderOverride, c.SystemPrompt, c.RetryConfig, boolInt(c.StableFirst), syncMode)
	if err != nil {
...
func (s *ChannelStore) Update(c *domain.Channel) error {
	...
	if _, err = tx.Exec(`UPDATE channels SET site_id=?, credential_id=?, name=?, base_url=?, models_csv=?, group_name=?, priority=?, weight=?, status=?, type_hint=?, max_reasoning_effort=?, payload_rules=?, max_concurrent=?, non_stream_timeout_seconds=?, stream_policy=?, proxy_url=?, header_override=?, system_prompt=?, retry_config=?, stable_first=?, model_sync_mode=?, updated_at=datetime('now') WHERE id=?`,
		c.SiteID, c.CredentialID, c.Name, c.BaseURL, c.ModelsCSV, c.GroupName, c.Priority, c.Weight, c.Status, c.TypeHint, c.MaxReasoningEffort, c.PayloadRules, c.MaxConcurrent, c.NonStreamTimeoutSeconds, c.StreamPolicy, c.ProxyURL, c.HeaderOverride, c.SystemPrompt, c.RetryConfig, boolInt(c.StableFirst), domain.NormalizeModelSyncMode(c.ModelSyncMode), c.ID); err != nil {
