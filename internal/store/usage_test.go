...
func TestChannelTagsBulkUpdate(t *testing.T) {
	db := openTestDB(t)
	siteID, _ := db.Site.Create(&domain.Site{Name: "s-tags", BaseURL: "https://t.example", Platform: "openai-compatible", Status: domain.StatusEnabled})
	credID, _ := db.Credential.Create(&domain.Credential{SiteID: siteID, Kind: "api_key", SecretEnc: []byte("enc"), Status: domain.StatusEnabled})
	// Three channels; two tagged "prod", one tagged "production" (LIKE anchor).
	_, _ = db.Channel.Create(&domain.Channel{SiteID: &siteID, CredentialID: &credID, Name: "a", Status: domain.StatusEnabled, Tags: "prod,us-east"})
	_, _ = db.Channel.Create(&domain.Channel{SiteID: &siteID, CredentialID: &credID, Name: "b", Status: domain.StatusEnabled, Tags: "prod"})
	_, _ = db.Channel.Create(&domain.Channel{SiteID: &siteID, CredentialID: &credID, Name: "c", Status: domain.StatusEnabled, Tags: "production"})

	// Bulk: raise weight + disable on tag "prod".
	weight, status := 500, domain.StatusDisabled
	affected, err := db.Channel.UpdateByTag("prod", domain.ChannelPatch{Weight: &weight, Status: &status})
	if err != nil {
		t.Fatal(err)
	}
	if affected != 2 {
		t.Fatalf("affected=%d want 2 (not the 'production' channel)", affected)
	}
	// Verify: a and b updated, c untouched.
	wants := map[string]struct {
		w int
		s string
	}{
		"a": {500, domain.StatusDisabled},
		"b": {500, domain.StatusDisabled},
		"c": {0, domain.StatusEnabled},
	}
	...
	for _, ch := range channels {
		if want, ok := wants[ch.Name]; ok {
			if ch.Weight != want.w || ch.Status != want.s {
				t.Fatalf("%s: weight=%d status=%s want %d/%s", ch.Name, ch.Weight, ch.Status, want.w, want.s)
			}
		}
	}
	// Tags normalize on write (dedupe + trim).
	_, _ = db.Channel.Create(&domain.Channel{SiteID: &siteID, CredentialID: &credID, Name: "d", Status: domain.StatusEnabled, Tags: " x , x , y "})
	channels, _ = db.Channel.List()
	for _, ch := range channels {
		if ch.Name == "d" {
			if ch.Tags != "x,y" {
				t.Fatalf("tags=%q want \"x,y\"", ch.Tags)
			}
		}
	}
